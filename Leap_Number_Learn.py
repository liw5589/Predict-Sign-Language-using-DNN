...
XLS = pd.read_csv('LeapTest/resultN.csv', encoding = 'euc-kr')
XLS = XLS.round(3)
...
model = tflearn.DNN(net,checkpoint_path="LeapTest/hidden(5,8)_Number_original.ckpt")
model.fit(x_train, y_train, validation_set=None, n_epoch=n_epochs,batch_size=batch_size)
model.save("LeapTest/hidden(5,8)_Number_original.tfl")
